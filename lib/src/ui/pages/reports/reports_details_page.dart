...
class ReportsDetailsPage extends StatefulWidget {
  const ReportsDetailsPage({required this.title, this.dateTimeRange, this.onDateRangeChanged, required this.items, Key? key}) : super(key: key);
...
class _ReportsDetailsPageState extends State<ReportsDetailsPage> {
  DateTimeRange _dateTimeRange = DateTimeRange(start: DateTime(2022), end: DateTime.now());
  ...
  void initState() {
    ...
    _dateController = TextEditingController(text: '${_dateTimeRange.start.toString().substring(0, 10)} - ${_dateTimeRange.end.toString().substring(0, 10)}');
  }
                    ...
                    Expanded(
                      child: TextField(
                        ...
                        onTap: ()async{
                          ...
                          if(dtr != null){
                            _dateTimeRange = dtr;
                            _dateController.text = '${_dateTimeRange.start.toString().substring(0, 10)} - ${_dateTimeRange.end.toString().substring(0, 10)}';
...
class ReportItemModel {
  ReportItemModel({
    required this.label, required this.amount, required this.expense,
  });
  final String label;
  final double amount;
  ...
  List<BarChartGroupData> toChartData (double width){
    ...
    for(var i=0; i<length; i++){
      l.add(BarChartGroupData(
        ...
        x: i,
        barRods: [
          BarChartRodData(toY: items[i].amount, color: Colors.green, width: width),
          BarChartRodData(toY: items[i].expense, color: Colors.redAccent, width: width)
        ]
      ));
    }
    return l;
